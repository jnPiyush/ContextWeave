...
Provides the `context-md run` command that bridges the sync Click CLI
to the async Agent Framework using asyncio.run().

Usage:
    context-md run 42 --role engineer
    context-md run 42 --type story
    context-md run 42 --dry-run
"""
...
def run_cmd(
    ...
    with the full Context.md 4-layer context architecture.

    Requires: pip install agent-framework --pre

    \b
    Examples:
        context-md run 42                          # Auto-detect workflow
        context-md run 42 --role engineer          # Single agent
        context-md run 42 --type epic              # Full epic workflow
        context-md run 42 --dry-run                # Preview workflow
        context-md run 42 --resume                 # Resume from thread
    """
    ...
    if not AGENT_FRAMEWORK_AVAILABLE:
        raise click.ClickException(
            ...
            "Or: pip install context-md[agent]"
        )

    repo_root = ctx.obj.get("repo_root")
    if not repo_root:
        raise click.ClickException(
            "Not in a Git repository with Context.md initialized.\n"
            "Run 'context-md init' first."
        )
...
async def _run_async(
    ...
    from context_md.framework.config import LLMProvider, load_llm_config
    from context_md.framework.context_provider import ContextMdProvider
    from context_md.framework.middleware import create_middleware_stack
    from context_md.framework.orchestrator import WorkflowOrchestrator, WorkflowType
    from context_md.framework.thread_store import GitThreadStore
    from context_md.framework.tools import ToolRegistry
    ...
    context_provider = ContextMdProvider(
    ...
    from context_md.framework.agents import AgentFactory
    ...
    # 6. Save thread state
    if thread_store:
        from context_md.framework.thread_store import ChatMessage
...
def _handle_dry_run(
    ...
    from context_md.framework.config import load_llm_config
    from context_md.framework.orchestrator import WorkflowOrchestrator, WorkflowType
    from context_md.framework.tools import ToolRegistry
