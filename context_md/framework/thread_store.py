...
Implements persistent conversation storage in .agent-context/threads/,
...
class GitThreadStore:
    ...
    Stores conversation threads as JSON files in:
        .agent-context/threads/thread-{issue}-{role}.json

    This enables:
    - Persistent conversation history across sessions
    - Git-trackable thread state
    - Integration with existing .agent-context directory
    """
    ...
    def _threads_dir(self) -> Path:
        return self.repo_root / ".agent-context" / self.THREADS_DIR
    ...
    def list_threads(repo_root: Path) -> List[Dict[str, Any]]:
        ...
        threads_dir = repo_root / ".agent-context" / GitThreadStore.THREADS_DIR
        threads = []
    ...
    def delete_thread(repo_root: Path, issue: int, role: str) -> bool:
        ...
        thread_file = (
            repo_root / ".agent-context" / GitThreadStore.THREADS_DIR
