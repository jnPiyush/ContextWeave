...
Usage:
    context-md subagent spawn <issue> --role <role>
    context-md subagent list
    context-md subagent status <issue>
    context-md subagent complete <issue>
"""
...
@click.pass_context
def subagent_cmd(ctx: click.Context) -> None:
...
def spawn_cmd(ctx: click.Context, issue: int, role: str, title: Optional[str]) -> None:
    ...
    if existing:
        raise click.ClickException(
            ...
            f"Use 'context-md subagent complete {issue}' to finish it first."
        )
    ...
    try:
        ...
        click.echo("")
        click.secho(f"✅ SubAgent spawned successfully!", fg="green")
        click.echo("")
        ...
        click.echo(f"  1. Generate context: context-md context generate {issue}")
        click.echo(f"  2. Work in worktree: cd {worktree_path}")
        click.echo(f"  3. When done: context-md subagent complete {issue}")
...
def status_cmd(ctx: click.Context, issue: int, as_json: bool) -> None:
    ...
    click.echo(f"  Worktree exists: {'✅' if worktree_exists else '❌'}")
...
def complete_cmd(ctx: click.Context, issue: int, force: bool, keep_branch: bool) -> None:
    ...
    if worktree_path.exists():
        try:
            ...
            if result.stdout.strip() and not force:
                raise click.ClickException(
                    f"Worktree has uncommitted changes. Commit or stash them first.\n"
                    f"Use --force to discard changes."
                )
    ...
    state.save()
    click.echo(f"  Updated state tracking")
    ...
    click.echo("")
    click.secho(f"✅ SubAgent completed for issue #{issue}", fg="green")
...
def recover_cmd(ctx: click.Context, issue: int) -> None:
    ...
    if not result.stdout.strip():
        raise click.ClickException(
            ...
            f"Use 'context-md subagent complete {issue} --force' to clean up state."
        )
    ...
    try:
        ...
        click.secho(f"✅ Worktree recovered at: {worktree_path}", fg="green")
    except subprocess.CalledProcessError as e:
