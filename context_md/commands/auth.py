...
# Scopes needed for Context.md operations
...
def auth_cmd(ctx: click.Context) -> None:
    ...
    Examples:
        ...
        context-md auth status    # Check authentication status
        context-md auth logout    # Clear stored credentials
    """
    pass
...
def login_cmd(ctx: click.Context, no_browser: bool) -> None:
    ...
    if state.github_token:
        ...
        if user:
            ...
            click.echo("")
            click.echo("To switch accounts, run: context-md auth logout")
            return
    ...
    click.echo("  3. Paste the code and authorize Context.md")
    click.echo("")
    ...
    click.echo("  - context-md sync --pull    # Sync issues from GitHub")
    click.echo("  - context-md sync --push    # Push branches to GitHub")
    click.echo("")
...
def status_cmd(ctx: click.Context, as_json: bool) -> None:
    ...
    if status_data["authenticated"]:
        ...
        click.echo("")
        click.echo("  Run: context-md auth login")
