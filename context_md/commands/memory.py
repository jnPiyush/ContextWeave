...
Usage:
    context-md memory show                    # Show memory summary
    context-md memory lessons                 # List lessons learned
    context-md memory lessons add             # Add a lesson
    context-md memory record <issue>          # Record execution outcome
    context-md memory metrics                 # Show success metrics
    context-md memory session <issue>         # Save/show session context
"""
...
def metrics_cmd(ctx: click.Context, role: Optional[str]) -> None:
    ...
    if failures:
        click.secho("⚠️  Common Failures", fg="yellow", bold=True)
        click.echo("")
        for f in failures:
