"""Tests for context_weave.framework.run -- CLI run command."""
...
class TestRunCommandFrameworkCheck:
    @patch("context_weave.framework.AGENT_FRAMEWORK_AVAILABLE", False)
...
class TestRunCommandDryRun:
    @patch("context_weave.framework.AGENT_FRAMEWORK_AVAILABLE", True)
...
class TestRunCommandExecution:
    @patch("context_weave.framework.AGENT_FRAMEWORK_AVAILABLE", True)
    @patch("context_weave.framework.run._run_async")
    def test_run_workflow(self, mock_run_async, runner, mock_repo):
        from context_weave.framework.orchestrator import WorkflowResult, WorkflowType
    ...
    def test_run_failure(self, mock_run_async, runner, mock_repo):
        from context_weave.framework.orchestrator import WorkflowResult
    ...
    @patch("context_weave.framework.AGENT_FRAMEWORK_AVAILABLE", True)
    @patch("context_weave.framework.run.asyncio")
