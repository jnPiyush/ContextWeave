...
class TestState:
    ...
    def test_default_state(self, temp_git_repo):
        ...
        assert state.github.enabled is False
...
class TestCLI:
    ...
    def test_version(self, runner):
        ...
        assert "context-weave" in result.output
    ...
    def test_init_command(self, runner, temp_git_repo):
        ...
        with runner.isolated_filesystem():
            ...
            assert Path(".context-weave/state.json").exists()
            assert Path(".context-weave/config.json").exists()
...
class TestWorkflows:
    ...
    def test_full_subagent_workflow(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            with patch("context_weave.commands.subagent.subprocess.run") as mock_run, \
                 patch("context_weave.state.State.set_branch_note", return_value=True):
    ...
    def test_issue_to_context_generation(self, runner):
        ...
        with runner.isolated_filesystem():
            ...
            output_path = Path(".context-weave") / "context-101.md"
