...
stored in .context-weave/config.json under the 'llm' key.
...
def load_llm_config(repo_root: Path) -> LLMConfig:
    """Load LLM config from .context-weave/config.json under the 'llm' key.
...
def save_llm_config(repo_root: Path, llm_config: LLMConfig) -> None:
    """Save LLM config to .context-weave/config.json under the 'llm' key.
...
def resolve_api_key(llm_config: LLMConfig) -> str:
    ...
    Priority:
        ...
        2. System keyring (service='context-weave', username=env_var_name)
    ...
    try:
        import keyring
        stored_key = keyring.get_password("context-weave", env_var)
        if stored_key:
            return stored_key
    ...
    raise ValueError(
        ...
        f"or store it with: keyring set context-weave {env_var}"
    )
