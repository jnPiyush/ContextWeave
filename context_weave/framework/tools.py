...
Wraps existing ContextWeave operations (subprocess calls, state management,
...
class ToolRegistry:
    """Registry of ContextWeave tools for Agent Framework agents.
    ...
    def _build_tool_map(self) -> Dict[str, Any]:
        ...
        async def read_file(path: str) -> str:
            ...
            from context_weave.security import PathValidator
        ...
        async def run_command(command: str, agent_role: str = "unknown") -> str:
            ...
            """
            from context_weave.security import CommandValidator
        ...
        async def get_context(issue: int) -> str:
            ...
            context_path = repo / ".context-weave" / f"context-{issue}.md"
            if context_path.exists():
                try:
                    return context_path.read_text(encoding="utf-8")
                except (OSError, UnicodeDecodeError) as e:
                    return f"Error reading context: {e}"
            return f"No context file found for issue #{issue}. Generate with: context-weave context generate {issue}"
