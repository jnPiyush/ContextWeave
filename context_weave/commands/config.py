"""
Config Command - View and modify ContextWeave configuration

Usage:
    context-weave config [key] [value]
    context-weave config mode [local|github|hybrid]
...
def config_cmd(ctx: click.Context, key: Optional[str], value: Optional[str],
    ...
    """View or modify ContextWeave configuration.

    Examples:
        context-weave config                    # Show current mode
        context-weave config --list             # List all config
        context-weave config mode               # Get mode value
        context-weave config mode github        # Set mode to github
        context-weave config hooks.pre_push     # Get specific setting
        context-weave config hooks.pre_push false  # Disable pre-push hook
    """
    ...
    if not key:
        ...
        click.echo("")
        click.echo("Use 'context-weave config --list' to see all settings")
        click.echo("Use 'context-weave config <key> <value>' to change a setting")
        return
