...
# Context.md - Advanced Context Management & Quality Orchestration

**Document ID**: PRD-CONTEXT-MD (Will be updated with issue number)
Version: 1.0
Status: Draft
...
1. Executive Summary
Overview
Context.md is a runtime context management and quality orchestration system that enables AI agents to achieve >95% success rate in production-ready code generation through intelligent context assembly, automated quality gates, and continuous learning.
...
Acceptance Criteria:
 ...
 Script generates .agent-context/context-{issue}.md per task
...
# .github/scripts/generate-context.sh <issue-number># 1. Fetch issue metadata (labels, type, status)# 2. Determine required skills based on labels# 3. Load complete role instructions# 4. Load all previous deliverables# 5. Load complete relevant skills (never strip)# 6. Validate completeness# 7. Output to .agent-context/context-{issue}.md
Priority: P0 (Critical)
...
Acceptance Criteria:
 ...
 Script generates .agent-context/plan-{issue}.md before code work
...
Acceptance Criteria:
 ...
 Generates .agent-context/learning-{date}.json report
...
Acceptance Criteria:
 ...
 Metrics stored in .agent-context/metrics-{date}.json
...
╔══════════════════════════════════════════════════════╗║  AgentX - Context Management Dashboard              ║║  Updated: 2026-01-29 14:35:00                       ║╠══════════════════════════════════════════════════════╣║  Active Issues: 5                                    ║║  ✓ PM: #456 (Ready) - Context.md PRD                ║║  ⚠ Engineer: #123 (In Progress, 18h) - JWT tests   ║║  ✓ Reviewer: #122 (In Review) - Auth middleware     ║║  ⚠ Architect: #457 (Ready, stuck) - OAuth design   ║║  ✓ UX: #458 (In Progress) - Login flow             ║╠══════════════════════════════════════════════════════╣║  Success Rate (30d): 85.0% (↑ from 78%)            ║║  Stuck Issues: 1 (escalated)                        ║║  Quality Gates: 88% pass rate                       ║╚══════════════════════════════════════════════════════╝
Priority: P2 (Medium)
...
- Each SubAgent gets `.agent-context/context-{issue}.md`
- Coordination via `.agent-context/subagent-status.json`
...
SubAgent-456 CAN access:
  ✓ .agent-context/context-456.md (own context)
  ✓ .agent-context/plan-456.md (own execution plan)
...
SubAgent-456 CANNOT access:
  ✗ .agent-context/context-457.md (other SubAgent's context)
  ✗ .agent-context/context-458.md (other SubAgent's context)
...
**Acceptance Criteria**:
- [ ] Status file `.agent-context/subagent-status.json` tracks all active SubAgents
...
- [ ] Results logged to `.agent-context/analysis-{issue}.json`
...
**User Story**:
As a **Developer**, I want **Context.md to work locally without GitHub** so that **I can use the system when offline or without GitHub access**.

**Acceptance Criteria**:
- [ ] System detects GitHub availability on startup
- [ ] If GitHub unavailable, switches to local-only mode
- [ ] Local issue tracking via `.agent-context/local-issues.json`
...
GitHub Unavailable (Local Mode):
  Issues → .agent-context/local-issues.json
  Status → .agent-context/local-status.json
...
# 2. Read .agent-context/local-issues.json
...
**Acceptance Criteria**:
- [ ] Every action logged to `.agent-context/audit-log.json`
...
- [ ] Certificate stored at `.agent-context/certificates/cert-{issue}.json`
...
System MUST generate .agent-context/context-{issue}.md per task
...
System MUST store plan at .agent-context/plan-{issue}.md
...
- System MUST provide local issue tracking via `.agent-context/local-issues.json`
...
- System MUST track SubAgent status in `.agent-context/subagent-status.json`
...
- System MUST log all agent actions to `.agent-context/audit-log.json`
...
Impact: Context.md extends but doesn't replace AGENTS.md
Change Required: Add Context.md references to workflow
...
Agent Dependencies
DEP-AGENT-1: Product Manager → Context.md (High)
...
DEP-AGENT-2: Engineer → Context.md (Critical)

Engineer uses generated context for execution
Engineer follows execution plan
Interface: .agent-context/context-{issue}.md, plan-{issue}.md
DEP-AGENT-3: Agent X → Context.md (High)
...
Description: .agent-context/ directory grows unbounded (100s of MB)
Mitigation:
...
Fallback: .agent-context/ in .gitignore, local-only
...
✅ .agent-context/context-{issue}.md generation working
...
Question: Should .agent-context/ be committed to Git or kept local?
Options:
...
Question: Should Context.md expose MCP tools for external integration?
Options:
