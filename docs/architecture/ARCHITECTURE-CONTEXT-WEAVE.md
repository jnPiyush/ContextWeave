# ContextWeave - Complete Architecture & Implementation Plan
...
**Status**: Approved  
**Related Documents**:
- [PRD-CONTEXT-WEAVE.md](../prd/PRD-CONTEXT-WEAVE.md)
- [ADR-CONTEXT-WEAVE.md](../adr/ADR-CONTEXT-WEAVE.md)
- [SPEC-CONTEXT-WEAVE.md](../specs/SPEC-CONTEXT-WEAVE.md)
...
ContextWeave is a runtime context management and quality orchestration system designed to achieve **>95% success rate** in AI agent production code generation through:
...
| **Interface** | CLI (`context-weave`) | Cross-platform, scriptable |
...
│  │                         LOCAL FILE SYSTEM                             │  │
│  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────┐   │  │
│  │  │ .context-weave/ │  │ .github/scripts/│  │ docs/dashboard/     │   │  │
...
│  │                                                                      │    │
│  │  Agent X           generate-context.sh       .context-weave/        │    │
│  │      │                     │                      │                 │    │
│  │      │──Assign Issue───────│                      │                 │    │
│  │      │                     │                      │                 │    │
│  │      │                     │──Fetch Issue─────────│                 │    │
...
└── .github/
    └── scripts/
        └── context-weave/                # CLI implementation
            └── context-weave.py          # Main CLI tool
```
...
│                                                                             │
│  WORKFLOW:                                                                  │
│    1. context-weave subagent spawn 456                                        │
...
# Run DoD validation
context-weave validate $ISSUE_NUM --dod
...
context-weave init --mode github
context-weave init --mode hybrid

# Check current mode
context-weave config mode
...
```
context-weave - Runtime context management for AI agents
...
Initialize ContextWeave in current repository:
...
#### issue (Local Mode)
```bash
context-weave issue create --title "Add JWT validation" --type story
context-weave issue list [--status in_progress]
context-weave issue show 456
context-weave issue close 456
...
#### context
```bash
context-weave context generate 456 [--output path]
context-weave context show 456
context-weave context refresh 456

# Generates .context-weave/context-456.md with:
# - Role instructions
# - Issue details
# - Relevant skills
# - Dependencies
...
context-weave subagent status 456
context-weave subagent complete 456

# Creates worktree at ../worktrees/456/
# Tracks in .context-weave/state.json
```

#### validate
```bash
context-weave validate 456 --task        # Validate task quality
context-weave validate 456 --pre-exec    # Pre-flight validation
context-weave validate 456 --dod         # Definition of Done
context-weave validate 456 --all         # All validations
...
Repository Root/
│
├── .context-weave/                    # Runtime state (git-ignored)
...
│   │   ├── context-manager/           # Core ContextWeave scripts
...
│   │
│   ├── prd/
│   │   └── PRD-CONTEXT-WEAVE.md
│   ├── adr/
│   │   └── ADR-CONTEXT-WEAVE.md
│   ├── specs/
│   │   └── SPEC-CONTEXT-WEAVE.md
│   └── architecture/
│       └── ARCHITECTURE-CONTEXT-WEAVE.md  # This document
│
└── .gitignore                         # Updated to exclude .context-weave/
```
...
│   11. Write context file to .context-weave/context-{issue}.md              │
│   12. Log generation to audit                                              │
│                                                                             │
│  OUTPUT:                                                                    │
│    • .context-weave/context-{issue}.md                                      │
...
*Generated by ContextWeave v1.0*
```
...
```json
{
  "$schema": "https://contextweave.dev/schemas/subagent-status-v1.json",
  ...
  "subagents": [
    {
      ...
      "plan_file": ".context-weave/plan-456.md",
...
```json
{
  "$schema": "https://contextweave.dev/schemas/audit-log-v1.json",
  "version": "1.0",
  "entries": [
    {
...
```json
{
  "$schema": "https://contextweave.dev/schemas/metrics-v1.json",
  "date": "2026-01-29",
  "period": "daily",
...
./install-context-weave.sh  # or install-context-weave.ps1 on Windows
...
│  │     ✓ Its own context file (.context-weave/context-{N}.md)          │   │
│  │     ✓ Its own plan file (.context-weave/plan-{N}.md)                │   │
...
│  Deliverables:                                                              │
│    • context-weave CLI tool (Python)                                          │
│    • init, config commands                                                 │
│    • Git hooks installation                                                │
│    • .context-weave/ directory structure                                   │
│  Success Criteria:                                                          │
│    • context-weave init works on Windows/Mac/Linux                            │
...
|----|------|----------|--------------|-------------|
| 1.1 | Create Python package structure | 2h | None | context_weave/ |
| 1.2 | Implement CLI entry point (Click) | 4h | 1.1 | context-weave command |
...
|-----------|------|---------------|
| **M1: CLI Foundation** | Week 2 | `context-weave init` works, hooks install |
...
| Risk | Contingency |
|------|-------------|
| R1: Disk space | `context-weave cleanup --all` command |
| R2: Hook compatibility | Provide hook disable option |
| R5: Corruption | `context-weave subagent recover` command |
| R6: Many branches | Archive old issues: `context-weave archive` |
...
```bash
Usage: context-weave init [OPTIONS]
...
```bash
Usage: context-weave subagent <command> [OPTIONS]
...
```bash
Usage: context-weave validate <issue> [OPTIONS]
...
Examples:
  ...
  context-weave validate 456 --all --verbose
```

### context-weave status

```bash
Usage: context-weave status [OPTIONS]

Show current ContextWeave status.
...
Examples:
  context-weave status
  context-weave status --watch
```

### context-weave sync (GitHub/Hybrid mode)

```bash
Usage: context-weave sync [OPTIONS]
